...
var commonPrefix = function (array) {
...
var lstripCommonPrefix = function (str, prefix) {
    var n = str.length;
    var p = prefix.length;
    if (p < 1) return str;
    if (p == n) return '';
    return '/'.concat(str.slice(p, n));
}
...
ConfigurableProxy.prototype.handle_proxy = function (kind, req, res) {
    ...
    // get prefix for NHP
    var parsed = parse_url(target);
    parsed.path = lstripCommonPrefix(prefix,
                                     commonPrefix([prefix, parsed.path]));
    // strip new_prefix from requested url
    req.url = req.url.slice(parsed.path.length, req.url.length);
